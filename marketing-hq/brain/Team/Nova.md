# Nova — Creative Director & Strategie

**Status:** 🟡 wacht op akkoord (2 approvals)
**Laatste run:** 2026-07-27 · ochtendcyclus
**Huidige taak:** 3 approvals openstaand (#1 advertorial ↗ kantelt, #2 SGL-mail,
#3 GroomGuard-test hersteld → monitoren)

> Identiteit, skills en guardrails: [`agents/nova.md`](../../agents/nova.md).

## Laatste activiteit
- **2026-07-27** · Approval #3 **gede-escaleerd** (GroomGuard-test hersteld
  naar 1,39 — monitoren i.p.v. acuut pauzeren). Advertorial-advies bijgesteld:
  3-daags 2,12 en stijgend, één bevestigingsdag van kanteling. **Hoofd-
  scheerapparaat-LP gepromoveerd naar Hypothese** (Brothers in Style 173 dagen
  / 128k impressies). Testimonial-static blijft prioriteit.
- **2026-07-26** · **Approval #3 aangemaakt**: runaway GroomGuard-testcampagne
  (€106/dag @ 0,56) pauzeren/cappen. Advertorial-advies ongewijzigd (3d-gem.
  1,24); advertorial-vernieuwing gekoppeld aan Ace & Taylor mechanisme-hook.
  Testimonial-static blijft prioriteit ↑↑ (BALZY schaalt door). Wellshine-
  spend-discipline als hypothese bevestigd.
- **2026-07-25** · Advertorial-advies **verhard naar níét uitvoeren** (3 dagen
  dalend, 3d-gem. 1,93); testimonial-static gepromoot naar **prioriteit ↑↑**
  na BALZY-topschalers (218k reach echte-klant-hook); Ace & Taylor
  mechanisme-hook genoteerd als referentie voor advertorial-vernieuwing.
- **2026-07-24** · Advertorial-advies ongewijzigd (te volatiel om te scalen,
  eerst creative verversen); Meridian-creative-explosie versterkt urgentie
  testimonial-static-productie; Wellshine-ommekeer als positief signaal.
- **2026-07-23** · Advertorial-advies ongewijzigd (afwachten tot 3-daags de
  3,0 raakt en vasthoudt; nu 2,30 en stijgend); Wellshine-spenddaling als
  positief signaal genoteerd; testimonial-static blijft productie-prioriteit.
- **2026-07-22** · Advertorial-advies bijgesteld na herstel 21/7 (nu
  afwachten i.p.v. afwijzen); testimonial-static-hypothese sterker
  (Meridian rank 6); hoofd-scheerapparaat-LP-idee blijft staan.
- **2026-07-21** · Advertorial-advies aangescherpt (**afwijzen** in huidige
  vorm; eerst creative/LP verversen); nieuw idee **dedicated
  hoofd-scheerapparaat-LP**; Wellshine-analyse aangevraagd bij [[Atlas]].
- **2026-07-20** · Testimonial-statics gepromoveerd naar **Hypothese**
  (2 dagen marktbevestiging); advertorial-caveat aangescherpt met het
  concrete 3-daags cijfer (2,93 < 3,0).
- **2026-07-19** · Bundel-test gepromoveerd naar **Hypothese** (2 dagen
  marktbevestiging); nieuw idee **testimonial-statics**; kanttekening van
  [[Atlas]] verwerkt in approval #1 (3-daags gemiddelde als conditie).
- **2026-07-18** · [[../Pipeline/Board|Pipeline]] gevuld met 3 concepten
  (bundel-test, UGC-video "3 minuten 0 irritatie", advertorial-uitbouw) met
  falsifieerbare hypotheses; 2 acties klaargezet in
  [[../Inbox/Approvals|Approvals]].
